...
def negamax(is_ai, depth, alpha, beta):
    # 游戏是否结束 | | 探索的递归深度是否到边界
    if game_win(list_my) or game_win(list_enemy) or depth == 0:
        return evaluation(is_ai)

    blank_list = list(set(chess_board).difference(set(list_all)))
    ...
    for next_step in blank_list:
        ...
        if is_ai:
            list_my.append(next_step)
        else:
            list_enemy.append(next_step)
        list_all.append(next_step)

        value = -negamax(not is_ai, depth - 1, -beta, -alpha)
        if is_ai:
            list_my.remove(next_step)
        else:
            list_enemy.remove(next_step)
        list_all.remove(next_step)
...
def main():
    ...
    while game_end == 0: #game_endame没有结束，就一直运行

        if change % 2 == first: #白棋根据first来进行判断
            pos = ai()
            ...
            if game_win(list_my):
                message = Text(Point(100, 100), "ai win.")
                message.draw(win)
                game_end = 1
            change = change + 1
            ...
            p2 = win.getMouse()
            if not ((round((p2.getX()) / GRID_WIDTH), round((p2.getY()) / GRID_WIDTH)) in list_all):

                a2 = round((p2.getX()) / GRID_WIDTH)
                b2 = round((p2.getY()) / GRID_WIDTH)
                list_enemy.append((a2, b2))
                list_all.append((a2, b2))

                piece = Circle(Point(GRID_WIDTH * a2, GRID_WIDTH * b2), 16)
                piece.setFill(enemycolor)
                piece.draw(win)
                if game_win(list_enemy):
                    message = Text(Point(100, 100), "human win.")
                    message.draw(win)
                    game_end = 1
